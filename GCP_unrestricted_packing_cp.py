...
def CP_model_unrestricted(params):
    NUM_COLORS, NUM_ITEMS, BIN_SIZE, DISCREPANCY, ITEM_SIZES, COLORS, NUM_BINS, Timelimit, SEED = params

    print("*** Running CP Unrestricted with instance NUM_COLORS{} NUM_ITEMS{} BIN_SIZE {} DISCREPANCY {} SEED {}".format(NUM_COLORS, NUM_ITEMS, BIN_SIZE, DISCREPANCY,SEED))
    ...
    try:
        ...
        if solution_checker(Xs, params, 'unrestricted_cp_binary'):
            write_to_global_cp(msol, mdl, 'unrestricted_binary')

    except Exception as err:
        print(err)
        write_to_global_failed(params, "unrestricted_cp_binary", is_bad_solution=False)
